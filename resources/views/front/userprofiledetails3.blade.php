            ...
            <div class="container">
                <div class="row">
                    <div class="column {{ ($userdetails->type == 2) ? 'medic':'clinic' }}">
                        ...
                        <img src="/uploads/userimage/{{$userdetails->id}}/{{$userdetails->image}}" alt="{{ $userdetails->name }}" style="max-width: 235px;">
                        @else
                    ...
                    <div class="column half">
                        <p><span>Horarios de atención</span>
                            ...
                            @if($usertimings->monStatus == 1)
                                <td>Lunes &nbsp;&nbsp;</td> <td>{{ str_replace('-', ' - ', $usertimings->monTiming) }} </td></tr>
                            @endif
                            @if($usertimings->tueStatus == 1)
                                <tr><td>Martes </td><td>{{ str_replace('-', ' - ', $usertimings->tueTiming) }} </td></tr>
                            @endif

                            @if($usertimings->wedStatus == 1)
                                <tr><td>Miércoles &nbsp;&nbsp; </td><td>{{ str_replace('-', ' - ', $usertimings->wedTiming) }} </td></tr>
                            @endif

                            @if($usertimings->thuStatus == 1)
                                <tr><td>Jueves &nbsp;&nbsp; </td><td>{{ str_replace('-', ' - ', $usertimings->thuTiming) }} </td></tr>
                            @endif

                            @if($usertimings->friStatus == 1)
                                <tr><td>Viernes &nbsp;&nbsp; </td><td>{{ str_replace('-', ' - ', $usertimings->friTiming) }} </td></tr>
                            @endif

                            @if($usertimings->satStatus == 1)
                                <tr><td>Sábado &nbsp;&nbsp; </td><td>{{ str_replace('-', ' - ', $usertimings->satTiming) }} </td></tr>
                            @endif

                            @if($usertimings->sunStatus == 1)
                                <tr><td>Domingo &nbsp;&nbsp; </td><td>{{ str_replace('-', ' - ', $usertimings->sunTiming) }} </td></tr>
                            @endif
							</table>
                        </p>
      ...
      <div class="row">
        <div class="column">
          <div class="registration">
            ...
            <p>Llena los campos para recibir por correo la información completa del médico o clínica. Y como nos gusta consentirte, además podrás recibir antes que nadie beneficios exclusivos como promociones, eventos y mucho más.</p>
            ...
            <form action="" method="" onsubmit="return saveReqInfo();" name="reqForm" id="reqForm" >
              ...
              <fieldset>
                <input type="text" placeholder="Correo electrónico"   email="true" id="req_email" name="req_email" />
              </fieldset>
              <fieldset>
                <select name="req_option" id="req_option"  ><option value="">¿Qué le quieres preguntar?</option><option value="1">Solicitar Costos</option><option value="2">Agendar Cita</option><option value="3">¿Soy candidato(a) al Tratamiento?</option><option value="4">Informacion Sobre Ultherapy</option></select>
              </fieldset>
              <fieldset>
                <textarea name="req_description" id="req_description" placeholder="Cuéntale a tu médico o clínica tu inquietud:"></textarea>
          ...
          <div class="success">
            ...
            <p>Acabamos de enviarte un correo con la información completa del médico o clínica y pronto recibirás una respuesta a tu consulta.</p>
